...
import smartsheet                                           # Needed to work with Smartsheet API
import time                                                 # Needed to make program halt for 5 seconds between uploads
from time import sleep                                      # sleep() is the function used to make program halt
import PySimpleGUI as sg                                    # Used for GUI, "sg" is shorter than "PySimpleUI" so import package as "sg" just gives it a nickname (kind of)
...
def  image_rename(image_path, xml_path, start_number):      # Function for renaming all images. needs to be passed an image path and an XML path
    ...
    for entry in os.listdir(image_path):                                # Iterate through the image directory given by user
    ...
    for x in range(0,len(views)):                                                                   # Iterate through all viewpoints
        if (start_number + x) < 10:                                                                 # If the index 'x' is less than 10
            temp_name = '0' + str(start_number + x) + '_' + view_names[x]                           # Append the index (with an extra 0 in front) to the front of the viewpoint name
            ...
            temp_name = str(start_number + x) + '_' + view_names[x]                                 # Append the index to the front of the viewpoint name
