...
def xml_to_excel(xml_path, new_path, new_name):             # Define function for exporting XML to excel
    ...
    folders = folder_level(root[0][0], 0, [])                                   # After function is defined, call it, passing in the first viewfolder, index of 0, and an empty array
    ...
    for x in range(0,len(cells)):                           # For loop to iterate through all items in cells array
        for y in range(0,len(cells[x])):                    # For loop to iterate through every item in each array that is stored in the cells array.
            ...
            sheet1.write(y,x,cells[x][y])                   # Write the information to the excel sheet 
    ...
    for x in range(0,len(view_names)):                      # Iterate through all viewnames
        ...
        for y in range(0,len(split_views)):                 # Iterate through the new array created by splitting the viewnames
            sheet1.write(x,max_lvl + y,split_views[y])      # Write that information to the sheet with the start column being the max level
...
options = ['XML to Excel and Image Rename', 'Smartsheet Image Upload']                                      # Options for the GUI 

form_option_1 = sg.FlexForm('XML to Excel and Image Rename')                                                # Set up form for XML to Excel and Image Rename
...
layout_option_1 = [                                                                                         # Set up layout for option 1
          ...
          [sg.Text('Image Folder', size=(18, 1)), sg.InputText(''), sg.FolderBrowse()],                     # Folder browse to get the image folder
...
while True:                                                                 # Indefinite while loop
    ...
    if button is None or button == 'Exit':                                  # If the user exits the GUI
        break                                                               # Break the while loop

    if values[0] == 'XML to Excel and Image Rename' and button == 'Enter':  # If the first option is chosen
        button_1, values_1 = form_option_1.Layout(layout_option_1).Read()   # Show the option 1 window and read the output
        xml_to_excel(values_1[0], values_1[2], values_1[3])                 # Call xml_to_excel function and pass the information given by user
        image_rename(values_1[1], values_1[0])                              # Call image_rename function and pass the information given by user
    ...
    if values[0] == 'Smartsheet Image Upload' and button == 'Enter':        # If the second option is chosen
        ...
        image_upload(values_2[0],int(values_2[1]),values_2[3],values_2[2])  # Call image_upload function and pass the information given by user
