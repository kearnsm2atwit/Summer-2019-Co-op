...
response = smart.Home.list_folders(include_all=True)
folders = response.data

workspace_response = smart.Workspaces.list_workspaces(include_all=True)
workspaces = workspace_response.data


options = []
for x in range (0,len(workspaces)):
  #print(workspaces[x].name)
  options.append(workspaces[x].name)


for x in range(0,len(folders)):
  #print(folders[x].name)
  options.append(folders[x].name)
  #folder = smart.Folders.get_folder(folders[x].id_)

for x in range(0,len(options)):
  print(options[x])


#print(folders.sheets[0].id_)
