...
def xml_to_excel(xml_path, new_path, new_name):             # Define function for exporting XML to excel
    ...
    for x in range(0,len(cells)):                           # For loop to iterate through all items in cells array
        ...
        for x in range(0,len(views)):                                   # Loop through all the views again
...
def  image_rename(image_path, xml_path):                    # Function for renaming all images. needs to be passed an image path and an XML path
    ...
    for x in range(0,len(views)):                      # Iterate through all viewpoints
        if x < 10:                                          # If the index 'x' is less than 10
            temp_name = '0' + str(x) + '_' + view_names[x]  # Append the index (with an extra 0 in front) to the front of the viewpoint name
            ...
            temp_name = str(x) + '_' + view_names[x]        # Append the index to the front of the viewpoint name
...
if values[0] == 'Image Rename' and button == 'Enter':                       # If the third option is chosen (These are out of order because Image Upload used to be part of operation 1)
    ...
    image_rename(values_3[1], values_3[0])                                  # Call the Image_rename function
