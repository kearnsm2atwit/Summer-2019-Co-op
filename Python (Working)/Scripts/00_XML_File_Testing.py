...
#Numbering the image names to retain the same order
for x in range(0,len(viewpoints)):                                                                     # Something like this can be used to rename all the images in a folder based on names of viewpoints
        if x < 10:
                viewpoints[x] = '0'+ str(x) + '_' + viewpoints[x]
              
        else:
                viewpoints[x] = str(x) + '_' + viewpoints[x]



# Renaming images in a folder based on viewpoint names
files = []
for entry in os.listdir(image_path):
        files.append(entry)
        
for x in range(0,len(files)):
        os.rename(image_path + '\\' + files[x], image_path + '\\' + viewpoints[x] + '.jpg')
